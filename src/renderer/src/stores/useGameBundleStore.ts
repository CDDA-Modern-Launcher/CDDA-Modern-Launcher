...
interface GameBundleStoreState extends IMountableState {
...
function applyGameState(set: (state: Partial<GameBundleStoreState>) => void, nextState: GameBundleState): void {
...
function toErrorState(error: unknown): GameBundleState {
    return { status: "error", message: getErrorMessage(error) };
}

function isFileOperationBlocked(get: () => GameBundleStoreState): boolean {
    return get().isFileOperationRunning;
}

function setResultError(set: (state: Partial<GameBundleStoreState>) => void, message: string): void {
...
export function selectIsGameFileOperationRunning(state: Pick<GameBundleStoreState, "isFileOperationRunning">): boolean {
    return state.isFileOperationRunning;
}
