...
import { BackupsDrawer, BackupStrip, DeleteBackupModal } from "./home/Backups";
import { getReleaseDisplayName, getUpdateReleases, isInstallRunning, type ReleaseNotesTarget, toUpdateReleaseNotesTarget } from "./home/homeUtils";
import { InstallOptionsModal, InstallProgressCard, InstallPrompt } from "./home/Install";
import { ReleaseNotesModal, VersionsDrawer } from "./home/VersionsDrawer";
import { VersionStrip } from "./home/VersionStrip";
import { BackupCreateButton, LastWorldButton, SaveStatusLine } from "./home/WorldActions";
...
import { LoadingWorkspaceView } from "@renderer/components/LoadingWorkspaceView";
import { InvalidWorkspaceView } from "@renderer/components/InvalidWorkspaceView";

export interface RepositoryGateProps {
    repository: WorkspaceStatus;
    isSelecting: boolean;
    onSelectRepository: () => void;
}

export function WorkspaceView({ repository, isSelecting, onSelectRepository }: RepositoryGateProps): React.JSX.Element {
    switch (repository.status) {
        case "invalid":
        case "unconfigured":
            return <InvalidWorkspaceView workspace={repository} isSelecting={isSelecting} onSelectRepositoryClick={onSelectRepository} />;
        case "loading":
            return <LoadingWorkspaceView path={repository.path} />;
        case "ready":
            return <ReadyRepository repository={repository} />;
    }
}

function ReadyRepository({ repository }: { repository: Extract<WorkspaceStatus, { status: "ready" }> }): React.JSX.Element {
    ...
    const [deleteBackupTarget, setDeleteBackupTarget] = useState<BackupInstanceInfo | null>(null);
    ...
    const [installModalRelease, setInstallModalRelease] = useState<GithubRelease | null>(null);
    ...
    const installRelease = async (releaseId?: string): Promise<void> => {
        setInstalling(true);
        try {
            const options: InstallDistributiveOptions = { releaseId, makeActive: true, copyUserdata, removeOlderInstalls: removeOldVersions };
