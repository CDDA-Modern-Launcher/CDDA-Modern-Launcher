...
const SETTINGS_FILE_NAME = 'cdda.launcher.settings.json'

export class LauncherSettingsStore {
  private readonly filePath = join(app.getPath('userData'), SETTINGS_FILE_NAME)

  async getRepositoryPath(): Promise<string | null> {
    const settings = await this.readSettings()
    return typeof settings.repositoryPath === 'string' && settings.repositoryPath.trim().length > 0
