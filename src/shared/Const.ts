export const WORKSPACE_CONFIG_FILE_NAME = "cataclysm-modern-launcher.jsonc";
export const BACKUP_INFO_FILE_NAME = "backup.json";
export const BACKUP_ARCHIVE_FILE_NAME = "world.zip";
export const DEFAULT_GAME_CHANNEL_ID = "cdda-experimental";
export const GAME_BUNDLES_DIRECTORY_NAME = "game-bundles";
export const USERDATA_DIRECTORY_NAME = "userdata";
export const DOWNLOADS_DIRECTORY_NAME = "downloads";
export const GAME_BUNDLE_MANIFEST_FILE_NAME = "game-bundle.json";
